...
class VaccinationValidator {
  ...
  validateName(name) {
    ...
    // Check for valid characters (any unicode letters, spaces, hyphens, apostrophes, and dots)
    // Allowing broader unicode improves support for non-English names
    const nameRegex = /^[\p{L}\s\-'.]+$/u;
    if (!nameRegex.test(name.trim())) {
      this.errors.push('Child name can only contain letters, spaces, hyphens, apostrophes, and dots');
      return false;
    }
